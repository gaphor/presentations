...
dev_stats = [45.8, 27.5, 26.6, 0.1, 0]
labels = ["Windows", "macOS", "Linux-based", "BSD", "Unknown"]

fig = make_subplots(rows=1, cols=2, specs=[[{"type": "domain"}, {"type": "domain"}]])
fig.add_trace(
    go.Pie(labels=labels, values=user_stats, name="Desktop Market Share"), 1, 1
)
fig.add_trace(
    go.Pie(labels=labels, values=dev_stats, name="Developers Primary OS"), 1, 2
)
